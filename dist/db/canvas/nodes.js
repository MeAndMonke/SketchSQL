...
async function loadCanvasState(db, canvasId) {
    ...
    let connectionRows = [];
    if (nodeIds.length > 0) {
        [rowRows] = await db.query('SELECT * FROM RowEntries WHERE nodeID IN (?) ORDER BY rowIndex ASC', [nodeIds]);
    ...
    connectionRows.forEach((conn) => {
        ...
        if (!Array.isArray(sourceRow.foreignKeyTo))
            sourceRow.foreignKeyTo = [];
...
router.get('/api/canvas/:canvasId', async (req, res) => {
...
router.post('/api/canvas/:canvasId/sync', async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < nodes.length; i += 1) {
            ...
            const [existingRows] = await db.query('SELECT id FROM RowEntries WHERE nodeID = ?', [nodeId]);
            ...
            for (let r = 0; r < (node.rows || []).length; r += 1) {
                ...
                if (rowId && rowId > 0) {
                    await db.query(`UPDATE RowEntries
                            SET rowIndex = ?, name = ?, type = ?, comment = ?, defaultValue = ?, indexType = ?, nullable = ?, autoIncrement = ?, isUnsigned = ?
                         WHERE id = ?`, [rowIndex, safeName, safeType, safeComment, safeDefault, safeIndexType, safeNullable, safeAutoInc, safeUnsigned, rowId]);
                }
                else {
                    const [rowResult] = await db.query(`INSERT INTO RowEntries (nodeID, rowIndex, name, type, comment, defaultValue, indexType, nullable, autoIncrement, isUnsigned)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [nodeId, rowIndex, safeName, safeType, safeComment, safeDefault, safeIndexType, safeNullable, safeAutoInc, safeUnsigned]);
                    rowId = rowResult.insertId;
                }
                keepIds.add(rowId);
                rowIdsForNode[rowIndex] = rowId;
            }
        ...
        for (let n = 0; n < nodes.length; n += 1) {
            const node = nodes[n];
            const sourceNodeId = nodeIdMap.get(node.id ?? `new-${n}`);
            ...
            for (let rowIdx = 0; rowIdx < (node.rows || []).length; rowIdx += 1) {
                ...
                for (const entry of entries) {
                    ...
                    await db.query('INSERT INTO Connections (startID, endID, relation, controlDotX, controlDotY) VALUES (?, ?, ?, ?, ?)', [fromRowId, toRowId, rel, midX, midY]);
                }
            }
        }
        await db.commit();
